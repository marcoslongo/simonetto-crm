...
async function TempoPorEtapaChart({ lojaIds, from, to }: { lojaIds: number[]; from?: string; to?: string }) {
  const data = await getTempoPorEtapa(lojaIds, from, to)
  return <ChartTempoPorEtapa data={data} />
}

async function FunilAtendenteChart({ lojaIds, from, to }: { lojaIds: number[]; from?: string; to?: string }) {
  const data = await getFunilPorAtendente(lojaIds, from, to)
  return <ChartFunilPorAtendente data={data} />
}

// ─── Insights automáticos ─────────────────────────────────────────────────────

async function InsightsBlock({ lojaIds, from, to }: { lojaIds: number[]; from?: string; to?: string }) {
  const [counts, colunas, vnr, conversao, etapas] = await Promise.all([
    getMultiLojaStatusFunil(lojaIds),
    getMultiLojaKanbanColumns(lojaIds),
    getVnrStats(lojaIds),
    getConversaoPorLoja(lojaIds, from, to),
    getTempoPorEtapa(lojaIds, from, to),
  ])
  ...
  if (convRate < 10 && totalAll > 10) {
    insights.push({ icon: TrendingDown, color: 'text-red-600 bg-red-50 border-red-200',
      text: `Taxa de conversão em ${convRate}% — abaixo do esperado. Revise o processo de abordagem.` })
  } else if (convRate >= 20) {
    insights.push({ icon: TrendingUp, color: 'text-emerald-600 bg-emerald-50 border-emerald-200',
  ...
  if (etapasAtivas.length > 0) {
    const gargalo = etapasAtivas.reduce((max, e) => e.tempo_medio_horas > max.tempo_medio_horas ? e : max)
    const dias = Math.round(gargalo.tempo_medio_horas / 24)
    if (dias >= 2) insights.push({ icon: AlertTriangle, color: 'text-amber-700 bg-amber-50 border-amber-200',
      text: `Gargalo: leads ficam em média ${dias} dia${dias > 1 ? 's' : ''} na etapa "${gargalo.label}".` })
  }

  if (vnr.motivos.length > 0) {
    const top = vnr.motivos[0]
    insights.push({ icon: BarChart2, color: 'text-slate-700 bg-slate-50 border-slate-200',
      text: `Principal motivo de perda: "${top.label}" (${top.pct}% das vendas não realizadas).` })
  }

  if (conversao.length > 1) {
    const best = [...conversao].sort((a, b) => b.taxa_conversao - a.taxa_conversao)[0]
    if (best.taxa_conversao > 0) insights.push({ icon: TrendingUp, color: 'text-blue-700 bg-blue-50 border-blue-200',
  ...
  if (naoAtendidos > 20) insights.push({ icon: AlertTriangle, color: 'text-red-600 bg-red-50 border-red-200',
    text: `${naoAtendidos} leads não atendidos acumulados. Risco de perda por demora no primeiro contato.` })

  if (insights.length === 0) insights.push({ icon: TrendingUp, color: 'text-slate-500 bg-slate-50 border-slate-200',
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-500">
            {from && to ? `Período: ${from} → ${to}` : 'Todos os dados históricos'}
          </p>
        </div>
      </div>
  ...
  return (
    <div className="space-y-8">
      ...
      {isFiltered && (
        <div className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5">
          ...
          <p className="text-sm text-blue-700 font-medium">
            Exibindo dados de <strong>{from}</strong> até <strong>{to}</strong>
          </p>
        </div>
      )}
      ...
      <section className="space-y-4">
        <SectionHeader step="1" title="O que aconteceu?" description="Volume de leads, conversão geral e evolução no tempo" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <Suspense key={`funil-${from}-${to}`} fallback={<ChartSkeleton />}>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <Suspense key={`atendente-${from}-${to}`} fallback={<ChartSkeleton />}>
            <FunilAtendenteChart lojaIds={lojaIds} from={from} to={to} />
          </Suspense>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <SectionHeader step="4" title="O que fazer agora?" description="Insights gerados automaticamente a partir dos seus dados" />
        <Suspense key={`insights-${from}-${to}`} fallback={<ChartSkeleton height="h-48" />}>
          <InsightsBlock lojaIds={lojaIds} from={from} to={to} />
        </Suspense>
      </section>
    </div>
  )
